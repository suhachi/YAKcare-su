...
export type DoseEvent = 'PRE' | 'MAIN' | 'CONFIRM' | 'DONE_TAP' | 'SNOOZED_TAP' | 'NOTYET_TAP' | 'TIMEOUT';
...
복용 인스턴스의 타입 정의와 알림 루프 로직을 담고 있습니다.

### 주요 타입

- `DoseStatus`: 복용 상태 (예정/대기중/스누즈됨/완료/누락)
- `DoseInstance`: 복용 인스턴스 인터페이스
- `IntakeContext`: 복용 맥락 (식전/식후/취침전)

### 주요 함수

- `canSnooze()`: 스누즈 가능 여부
- `canMarkDone()`: 완료 처리 가능 여부
- `shouldAutoMiss()`: 자동 누락 처리 여부
- `getAlertType()`: 알림 타입 결정
